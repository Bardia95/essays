...
- Metadata::
    - Target Date:: [[April 24th, 2020]]
    - Tags:: #Clojure #Bitcoin #Math #Essays #[[Finite Fields]]
    - Status:: #[[In progress]]
    - Link:: [Finite Fields — Essays by Bardia](https://essays.bardiapourvakil.com/posts-output/2020-04-21-finite-fields/)
- # Draft
...
so after $$t$$ iterations, the probability no composite will be found with a composite number is at most $$\leq 1/2^t$$. So after $$50$$ iterations, the probability of mistakenly outputting a prime is $$1$$ in $$1125899906842624$$.
    ...
    ```(defn modpow 
  "b^e mod m, Integer Integer Integer -> Integer"
  [b e m]
...
(defn prime? 
  ...
  (every? true? (repeat 50 #(fermat-test p)))))```
    We first define a modulo exponential function `modpow`, then the actual primality test `fermat-test`, and finally the `prime?` function, which runs `fermat-test` 50 times to be sure that our input is truly prime.
...
(make-field-element 12 19)
(make-field-element 12 18)
(make-field-element -3 19)```
